...
Use a single agent as the default execution model. The agent may create tasks for traceability, but the skill should not require a specific platform, CLI, or parallel worker model.
...
When a task is useful, write a concise PRD with these sections:

```markdown
# Fill <package-or-layer> Specs

## Goal
Write project-specific `.daedalus/spec/` guidance for <scope>.
...
- `.daedalus/spec/.../index.md`
- `.daedalus/spec/.../<topic>.md`
